...
# ==============================================================================
# CORE RESOLUTION LOGIC (Handles ZoneLists, Spaces, and Zones)
# ==============================================================================
...
def _resolve_targets(building: besos.IDF_class) -> List[Dict[str, str]]:
    ...
    Handles ZoneList expansion and Space->Zone mapping.

    Returns a list of dictionaries with:
      - 'df_key': The key for user input (e.g., "ZoneName PeopleName")
      - 'ems_suffix': Sanitized suffix for EMS variables (e.g., "ZoneName_PeopleName")
      - 'sensor_key': The key for the EMS Sensor (People Name)
      - 'zone_name': The target Zone Name for Actuators/Schedules
    """
    ...
    # Map Space -> Zone (only if Spaces exist)
    space_to_zone = {}
    if not is_legacy:
        for s in building.idfobjects['SPACE']:
            space_to_zone[s.Name.upper()] = s.Zone_Name

    # Helper to extract zones from a ZoneList object (handling extensible fields)
    def get_zones_from_zl(zl_obj):
        z_names = []
        # Eppy extensible fields usually follow pattern Zone_X_Name
        for i in range(1, 500):
            try:
                # Try to access the field. Eppy might raise error or return empty if not found
                z = zl_obj[f"Zone_{i}_Name"]
                if z:
                    z_names.append(z)
                else:
                    break
    ...
    for people in building.idfobjects['PEOPLE']:
        # Robustly find the container name
        container_name = ""
        try:
        ...
        # CASE A: Assigned to a ZONELIST (Expand it!)
        if c_name_upper in zone_lists:
            zl_obj = zone_lists[c_name_upper]
            zones_in_list = get_zones_from_zl(zl_obj)

            for z in zones_in_list:
                # Pattern requested: "ZoneName PeopleName"
                targets.append({
                    'df_key': f"{z} {p_name}",
                    'ems_suffix': f"{z}_{p_name}".replace(' ', '_').replace(':', '_'),
                    'sensor_key': p_name,
                    'zone_name': z
                })

        # CASE B: Assigned to a SPACE (Find parent Zone)
        elif c_name_upper in space_to_zone:
            parent_zone = space_to_zone[c_name_upper]
            # Pattern: "SpaceName PeopleName"
            targets.append({
                'df_key': f"{container_name} {p_name}",
                'ems_suffix': f"{container_name}_{p_name}".replace(' ', '_').replace(':', '_'),
                'sensor_key': p_name,
                'zone_name': parent_zone
            })

        # CASE C: Assigned to a ZONE (Direct)
        else:
            if is_legacy:
                # Legacy Pattern: "ZoneName" (Keep compatibility)
                targets.append({
                    'df_key': container_name,
                    'ems_suffix': container_name.replace(':', '_').replace(' ', '_'),
                    'sensor_key': p_name,
                    'zone_name': container_name
                })
            else:
                # Modern Pattern: "ZoneName PeopleName" (To ensure uniqueness)
                targets.append({
                    'df_key': f"{container_name} {p_name}",
                    'ems_suffix': f"{container_name}_{p_name}".replace(' ', '_').replace(':', '_'),
                    'sensor_key': p_name,
