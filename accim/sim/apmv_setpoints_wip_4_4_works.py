...
def _sanitize_ems_name(name: str) -> str:
    """
    Sanitizes names for EMS variables.
    Replaces ANY character that is not a letter, number, or underscore with '_'.
    This covers spaces, colons, hyphens, dots, slashes, parentheses, etc.
    """
    # Regex: [^a-zA-Z0-9_] matches any character that is NOT alphanumeric or underscore
...
def _resolve_targets(building: besos.IDF_class) -> List[Dict[str, str]]:
    ...
    1. GLOBAL CHECK: Are there SPACE or SPACELIST objects in the model?
       YES -> ALL targets resolve to 'SpaceName PeopleName'.
              (Even if assigned to ZoneList, we find the spaces inside those zones).

    2. ELSE: Are there ZONELIST objects in the model?
       YES -> If assigned to ZoneList -> 'ZoneName PeopleName'.
              If assigned to Zone -> 'ZoneName' (Sensor: PeopleName).

    3. ELSE:
       -> 'ZoneName' (Sensor: PeopleName).
    """
    targets = []

    # --- 1. BUILD LOOKUP DICTIONARIES ---

    # Check existence of types (Robust against version)
    has_spaces = False
    try:
    ...
    # Dictionaries
    zone_lists = {}  # Name -> Object
    space_lists = {}  # Name -> Object
    space_to_zone = {}  # SpaceName -> ZoneName
    zone_to_spaces = {}  # ZoneName -> List[SpaceName]
    ...
    if has_spaces:
        try:
            ...
            for s in building.idfobjects['SPACE']:
                s_name = s.Name.strip()  # Keep original case for value
                s_key = s_name.upper()
    ...
    # Helper to extract items from extensible lists
    ...
    for people in building.idfobjects['PEOPLE']:
        ...
        # ======================================================================
        # SCENARIO 1: MODEL HAS SPACES (The "Modern" Path)
        # ======================================================================
        if has_spaces:
            # We must resolve WHATEVER the container is down to a list of Spaces.
            target_spaces = []  # List of tuples (SpaceName, ParentZoneName)

            # A. Assigned to SPACELIST
            ...
            # GENERATE TARGETS FOR SPACES
            for s_name, z_name in target_spaces:
                full_key = f"{s_name} {p_name}"
                targets.append({
                    'df_key': full_key,
                    'ems_suffix': _sanitize_ems_name(f"{s_name}_{p_name}"),
                    'sensor_key': full_key,  # Pattern: SpaceName PeopleName
                    'zone_name': z_name
                })

        # ======================================================================
        # SCENARIO 2: NO SPACES, BUT ZONELIST EXISTS (The "Intermediate" Path)
        # ======================================================================
        elif has_zonelists and c_name_upper in zone_lists:
            # Assigned to ZONELIST -> Expand to Zones
            zl_obj = zone_lists[c_name_upper]
            z_names = get_items_from_list(zl_obj, "Zone")
            ...
            for z in z_names:
                ...
                targets.append({
                    ...
                    'sensor_key': full_key,  # Pattern: ZoneName PeopleName
                    'zone_name': z
                })

        # ======================================================================
        # SCENARIO 3: SIMPLE ZONE ASSIGNMENT (Legacy/Simple Path)
        # ======================================================================
        else:
            ...
            targets.append({
                ...
                'sensor_key': p_name,  # Pattern: PeopleName (Original)
